.data
    ...
    crescente: .asciiz "\nf)Ordem crescente: "
    decrescente: .asciiz "\ng)Ordem decrescente: "
    numCaracteres: .asciiz "\ni)O numero de caracteres no arquivo: "

.text

	li $s2, 0		# $s2 é o somatorio final
	...
	main:
		...
		syscall
        	...
        	Alocacao:
            	...
            	mul $a0, $t0, 4         # $a0 recebe a quantidade de caracteres * 4 bytes
        ...
        LetraB:
			...
			lw $a0, ($a3)		# $a0 recebe o menor valor
			li $v0, 1	
			syscall
		...
		LetraF:
            ...
            jal Escrita_Vetor			# Escreve o vetor na ordem crescente
	...
	contagem:
        	...
        	addi $t0, $t0, 1
        	bnez $v0, contagem  		# Se $v0 é diferente do final do arquivo
	...
	percorre:
		...
		beq $t0, 13, percorre
		beq $t0, 10, newLine
		addi $t0, $t0, -48		# Subtrai 48 do caractere para obter o inteiro correspondente ao valor
		mul $t1, $t1, 10
		add $t1, $t1, $t0
		...
		newLine:
			...
			mul $t8, $t9, 4       # $t8 recebe (posicao*4bytes)
            add $t8, $t8, $a3      # $t8 recebe a soma do endereco base do vetor + posicao calculada
            sw $t1, ($t8)        # Armazena o valor no vetor
            li $t1, 0
            addi $t9, $t9, 1        # i++
	...
	LetraCDEH:
        ...
        Loop:
            ...
            beq $t0, $zero, Par     # Se o resto for zero o numero eh par 
        ...
        subi $t3, $t9, 1     # Subtrai 1 do valor de $a1(indice j)
       	...
       	Laco1:       # Funciona como um for(j = N-1 ; j>=1 ; j--)
            ...
            Laco2:     # Funciona como: for(i = 0 ; i<j ; i++)
                ...
                lw $s1, ($t0)   # $s1 recebe o valor armazenado no endereco $t0(elemento i+1 do vetor)
